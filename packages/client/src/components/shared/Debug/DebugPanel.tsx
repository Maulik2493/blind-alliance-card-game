...
export function DebugPanel() {
  ...
  return (
    <div className="p-3 text-xs font-mono bg-gray-50 space-y-4">
      ...
      {/* 1. GAME INFO */}
      <section>
        <h3 className="text-gray-500 uppercase text-xs tracking-wider mb-1">Game Info</h3>
      ...
      {/* 2. REMOVED CARDS */}
      <section>
        <h3 className="text-gray-500 uppercase text-xs tracking-wider mb-1">Removed Cards</h3>
        {removedCards.length === 0 ? (
          <span className="text-gray-400">None removed</span>
        ) : (
          <div className="flex flex-wrap gap-1">
            {removedCards.map((c, i) => (
              <span key={i} className="bg-gray-200 px-1.5 py-0.5 rounded text-gray-600">
      ...
      {/* 3. PLAYERS TABLE */}
      <section>
        <h3 className="text-gray-500 uppercase text-xs tracking-wider mb-1">Players</h3>
        <table className="w-full text-left">
          ...
          <tbody>
            {players.map((p, i) => {
              const teamColor =
                p.team === 'bidder' ? 'text-blue-600' :
                p.team === 'opposition' ? 'text-red-500' :
                'text-gray-400';
              const isCurrent = i === currentPlayerIndex;
              const isBidder = p.id === bidderId;
              const isMe = p.id === myPlayerId;
              return (
                <tr key={p.id} className={isMe ? 'bg-amber-50' : ''}>
      ...
      {/* 4. CURRENT TRICK */}
      <section>
        <h3 className="text-gray-500 uppercase text-xs tracking-wider mb-1">Current Trick</h3>
        {currentTrick ? (
          ...
          <span className="text-gray-400">No trick in progress</span>
        )}
      </section>

      {/* 5. TEAMMATE CONDITIONS */}
      <section>
        <h3 className="text-gray-500 uppercase text-xs tracking-wider mb-1">Teammate Conditions</h3>
      ...
      {/* 6. BID HISTORY */}
      <section>
        <h3 className="text-gray-500 uppercase text-xs tracking-wider mb-1">Bid History</h3>
        {bids.length === 0 ? (
          ...
          <div className="space-y-0.5">
            {bids.map((bid, i) => {
              ...
              return (
                <div key={i} className={isHighest ? 'text-amber-600' : 'text-gray-600'}>
